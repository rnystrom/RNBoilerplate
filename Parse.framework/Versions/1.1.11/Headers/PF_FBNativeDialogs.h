...
/*!
 ...
 being displayed.
 */
typedef void (^PF_FBShareDialogHandler)(PF_FBNativeDialogResult result, NSError *error);

/*!
 @class PF_FBNativeDialogs
 
 @abstract
 Provides methods to display native (i.e., non-Web-based) dialogs to the user.
 Currently the iOS 6 sharing dialog is supported.
 */
@interface PF_FBNativeDialogs : NSObject
...
/*!
 ...
 current active session returned by [PF_FBSession activeSession] will be used to determine
...
+ (BOOL)presentShareDialogModallyFrom:(UIViewController*)viewController
                          initialText:(NSString*)initialText
                                image:(UIImage*)image
                                  url:(NSURL*)url
                              handler:(PF_FBShareDialogHandler)handler;
...
/*!
 ...
 <PF_FBSession> may be specified, or nil may be passed to indicate that the current
 ...
 @param session     The <PF_FBSession> to use to determine whether or not the user has been
 ...
 */
+ (BOOL)canPresentShareDialogWithSession:(PF_FBSession*)session;
